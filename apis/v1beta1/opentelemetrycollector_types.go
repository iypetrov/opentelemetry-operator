...
// +kubebuilder:validation:XValidation:rule="!(self.mode != 'statefulset' && has(self.volumeClaimTemplates) && size(self.volumeClaimTemplates) > 0)",message="the OpenTelemetry Collector mode does not support the attribute 'volumeClaimTemplates' unless mode is 'statefulset'"
// +kubebuilder:validation:XValidation:rule="!(self.mode != 'statefulset' && has(self.persistentVolumeClaimRetentionPolicy))",message="the OpenTelemetry Collector mode does not support the attribute 'persistentVolumeClaimRetentionPolicy' unless mode is 'statefulset'"
// +kubebuilder:validation:XValidation:rule="!(self.mode != 'daemonset' && has(self.daemonSetUpdateStrategy) && has(self.daemonSetUpdateStrategy.type) && self.daemonSetUpdateStrategy.type != '')",message="the OpenTelemetry Collector mode does not support the attribute 'updateStrategy' unless mode is 'daemonset'"
// +kubebuilder:validation:XValidation:rule="!(self.mode != 'deployment' && has(self.deploymentUpdateStrategy) && has(self.deploymentUpdateStrategy.type) && self.deploymentUpdateStrategy.type != '')",message="the OpenTelemetry Collector mode does not support the attribute 'deploymentUpdateStrategy' unless mode is 'deployment'"
// +kubebuilder:validation:XValidation:rule="!(has(self.ingress) && has(self.ingress.type) && self.ingress.type == 'ingress' && self.mode == 'sidecar')",message="the OpenTelemetry Spec Ingress configuration is incorrect. Ingress can only be used in combination with the modes: deployment, daemonset, statefulset"
// +kubebuilder:validation:XValidation:rule="!(has(self.ingress) && has(self.ingress.ruleType) && self.ingress.ruleType == 'subdomain' && (!has(self.ingress.hostname) || self.ingress.hostname == '' || self.ingress.hostname == '*'))",message="a valid Ingress hostname has to be defined for subdomain ruleType"
// +kubebuilder:validation:XValidation:rule="!(has(self.autoscaler) && has(self.autoscaler.minReplicas) && !has(self.autoscaler.maxReplicas))",message="spec.autoscaler.maxReplicas must be set when spec.autoscaler.minReplicas is set"
// +kubebuilder:validation:XValidation:rule="!(has(self.replicas) && has(self.autoscaler) && has(self.autoscaler.maxReplicas) && self.replicas > self.autoscaler.maxReplicas)",message="the OpenTelemetry Spec autoscale configuration is incorrect, replicas must not be greater than maxReplicas"
// +kubebuilder:validation:XValidation:rule="!(has(self.autoscaler) && has(self.autoscaler.minReplicas) && has(self.autoscaler.maxReplicas) && self.autoscaler.minReplicas > self.autoscaler.maxReplicas)",message="the OpenTelemetry Spec autoscale configuration is incorrect, minReplicas must not be greater than maxReplicas"
// +kubebuilder:validation:XValidation:rule="self.mode == oldSelf.mode",message="the OpenTelemetry Collector mode is immutable; create a new collector with the desired mode instead"
